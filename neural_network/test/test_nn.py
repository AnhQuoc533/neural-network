...
def test_logistic_regression():
    ...
    # Check the costs
    assert pytest.approx(model.costs[0]) == 0.6931471805599453
    assert pytest.approx(model.costs[1]) == 0.5845083636993085
    assert pytest.approx(model.costs[2]) == 0.4669490409465547
    assert pytest.approx(model.costs[3]) == 0.3760068669480207
    assert pytest.approx(model.costs[4]) == 0.3314632893282512

    assert pytest.approx(model.costs[-1]) == 0.14087207570310165
...
def test_shallow_nn_2():
    ...
    model = NeuralNetwork([neurons, 1])
    np.random.seed(1)
    weight = {'W1': np.random.randn(neurons, train_set_x.shape[1]).T * 0.01,
              'W2': np.random.randn(neurons, 1) * 0.01}
    ...
    assert model.costs[2] == 0.6325140647912678
...
def test_mini_batch():
    ...
    model.fit(moons_X, moons_y, learning_rate=0.0007, epochs=5000, step=1000, batch_size=64, shuffle=True)
    ...
    assert pytest.approx(model.accuracy_score(moons_X, moons_y)) == 0.8066666666666666


def test_momentum_optimizer():
    model = NeuralNetwork([5, 2, 1], seed=3)
    model.fit(moons_X, moons_y,
              learning_rate=.0007,
              epochs=5000, step=1000,
              batch_size=64, shuffle=True,
              optimizer='momentum')

    assert pytest.approx(model.costs) == [0.7590075570464446, 0.5979284827243367, 0.5318105436146895,
                                          0.4940913450754209, 0.4635575132244337]
    assert pytest.approx(model.accuracy_score(moons_X, moons_y)) == 0.8066666666666666
...
def test_rmsprop_optimizer():
    ...
    assert pytest.approx(model.costs) == [0.7366576114998906, 0.1817141962462617, 0.1427976235658836,
                                          0.1410125678891975, 0.1421516720403639]
    assert pytest.approx(model.accuracy_score(moons_X, moons_y)) == 0.9433333333333334
...
def test_adam_optimizer():
    ...
    assert pytest.approx(model.costs) == [0.7579587937629741, 0.2540861504539721, 0.1459760758188075,
                                          0.1423536136409726, 0.1420671741058317]
    assert pytest.approx(model.accuracy_score(moons_X, moons_y)) == 0.9433333333333334
